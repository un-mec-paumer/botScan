...
const updateAlertRoute: FastifyPluginAsync = async (fastify) => {
  ...
  const schema: FastifySchema = {
    ...
    description: "Update alert with work and user ids. Non-specified params keeps the current value, false otherwise",
    tags: ['alerts'],
    security: [{ bearerAuth: [] }],
    body: UpdateAlertDto,
    response: {
  ...
  fastify.patch<{ Body: UpdateAlertDtoType }>(
    ...
    async (request, reply) => {
      try {
        ...
        const alert = await alertService.updateAlert(userId, workId, mangaAlert, animeAlert);
